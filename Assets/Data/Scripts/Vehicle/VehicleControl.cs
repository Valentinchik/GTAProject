...
public class VehicleControl : MonoBehaviour
{
	...
	public AudioSource audio_skid;

	public float suspensionDistance = 0.5f; // amount of movement in suspension
    public float springs = 7000.0f; // suspension springs
    public float dampers = 10f; // how much damping the suspension has
    ...
    public float brakeTorque = 1000f; // the power of the braks (per wheel)
    public float wheelWeight = 3f; // the weight of a wheel
    public Vector3 shiftCentre = new Vector3(0.0f, -0.8f, 0.0f); // offset of centre of mass

    public float maxSteerAngle = 25.0f; // max angle of steering wheels
    ...
    WheelData SetWheelParams(Transform wheel, float maxSteer, bool motor, float curY)
    {
		...
		if (AutomaticWheelRadius)wheelRadius=wheel.FindChild("wheel").GetComponent<Renderer>().bounds.size.y / 2;
    ...
    void FixedUpdate()
    {
        ...
        if (carSteer) carSteer.localEulerAngles = new Vector3(0, 0, -steer * 100);
	...
	void SkidAudio(WheelCollider _wheel){
		WheelHit temp_graund_hut;
		_wheel.GetGroundHit(out temp_graund_hut);
